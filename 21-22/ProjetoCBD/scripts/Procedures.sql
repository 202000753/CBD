...
CREATE or alter PROCEDURE insertNewStudent (@number int,
									@gender nvarchar,
									@birthdate datetime,
									@addressType bit,
									@famSize bit,
...
set @currentTime = CONVERT (nvarchar ,CONVERT (TIME, CURRENT_TIMESTAMP))
	...
	declare @userEEID int
	declare @guardianID int
	declare @userAlunoID int
	declare @addressID int

	set @SQL = '
	CREATE LOGIN LEE' + CAST(@number AS varchar) + ' with password = ' + char(39) + 'PASSWORD' + char(39) + ';
	CREATE USER UserEE' + CAST(@number AS varchar) + ' FOR LOGIN LEE' + CAST(@number AS varchar) + ';
	EXEC sp_addrolemember ' + char(39) + 'utilizador' + char(39) + ', ' + char(39) + 'UserEE' + CAST(@number AS varchar) + '' + char(39) + ';'
	EXEC(@SQL)

	insert into ActiveData.Sys_User (Mail, name, Password) values ('emailee'+CAST(@number AS varchar)+'@gmail.com', 'EE' + CAST(@number AS varchar), dbo.fnHashPassword('PASSWORD'))
	set @userEEID = (select UserID from ActiveData.Sys_User where Mail like 'emailee'+CAST(@number AS varchar)+'@gmail.com')

	insert into ActiveData.Guardian(UserID) values (@userEEID)
	set @guardianID = (select GuardianID from ActiveData.Guardian where UserID = @userEEID)

	set @SQL = '
	CREATE LOGIN LAluno' + CAST(@number AS varchar) + ' with password = ' + char(39) + 'PASSWORD' + char(39) + ';
	CREATE USER UserAluno' + CAST(@number AS varchar) + ' FOR LOGIN LAluno' + CAST(@number AS varchar) + ';
	EXEC sp_addrolemember ' + char(39) + 'utilizador' + char(39) + ', ' + char(39) + 'UserAluno' + CAST(@number AS varchar) + '' + char(39) + ';'
	EXEC(@SQL)

	insert into ActiveData.Sys_User (Mail, name, Password) values ('emailaluno'+CAST(@number AS varchar)+'@gmail.com', 'Aluno' + CAST(@number AS varchar), dbo.fnHashPassword('PASSWORD'))
	set @userAlunoID = (select UserID from ActiveData.Sys_User where Mail like 'emailaluno'+CAST(@number AS varchar)+'@gmail.com')
	
	insert into ActiveData.Student (UserID , BirthDate, Gender,FamSize, Pstatus, Medu, Fedu, Mjob, Fjob, Reason, Guardian_Relation, TravelTime, StudyTime, Schoolsup, Famsup, Paid, Activities, Nursery, Higher, Internet, Romatic, Famrel, Freetime, Goout, Walc, Dalc, Health, SchoolID, GuardianID) 
		values (@userAlunoID, @birthdate, @gender, @famSize, @Pstatus, @Medu, @Fedu, @Mjob, @Fjob, @reason, @guardianRelation, @traveltime, @studytime, @schoolsup, @famsup, @paid, @activities, @nursery, @higher, @internet, @romantic, @famrel, @freetime, @goout, @Dalc, @Walc, @health, @schoolID, @guardianID)
	...
	insert into ActiveData.UserAddress (UserID, AddressID, IsMainAddress, AddressType) values (@userAlunoID, @addressID, 1, @addressType)
	insert into ActiveData.UserAddress (UserID, AddressID, IsMainAddress, AddressType) values (@userEEID, @addressID, 1, @addressType)
END
go
...
Create or alter procedure tokenPasswordChange (@mail nvarchar(70), @tokenNumber nvarchar (10), @newPassword3 nvarchar(32), @newPassword4 nvarchar(32))
as
BEGIN
declare @tempoParaReset smallint
set @tempoParaReset = 60;
declare @tempoToken datetime 
declare @newmail nvarchar(70)
set @newmail=@mail
declare @password3 nvarchar(32)
set @password3=@newPassword3
declare @password4 nvarchar(32)
set @password4=@newPassword4
	IF EXISTS (select * from ActiveData.Sys_User where Mail = @mail)
	BEGIN
		...
		BEGIN
			...
			IF (DATEDIFF (MINUTE, @tempoToken, CURRENT_TIMESTAMP) <= @tempoParaReset )
				BEGIN
					exec changePassword @mail = @newmail, @newPassword1 = @password3, @newPassword2 = @password4
				END
			ELSE
				BEGIN
...
Create or alter procedure changeToNewPassword (@mail nvarchar(70),@oldPassword nvarchar(32) , @newPassword3 nvarchar(32), @newPassword4 nvarchar(32))
as
BEGIN
...
BEGIN
	...
	SET @StudentID = CONVERT(int, @StudentID_curs)
		...
		declare @userAlunoID int
		declare @alunoID int
		set @userAlunoID = (select UserID from ActiveData.Sys_User where Mail like 'emailaluno'+CAST(@StudentID AS varchar)+'@gmail.com')
		set @alunoID = (select StudentID from ActiveData.Student where UserID = @userAlunoID)
		--print (@alunoID)
		...
		exec insertRowCloosed_Course @schoolyear = @Year, @subjectID = @SubjectID, @studentID = @alunoID, @failures = @Failures , @absences = @Absences, @P1_grade =@P1, @P2_grade = @P2, @P3_grade = @P3
		FETCH next from StudentNumber_cursor  into @StudentID_curs
	END
