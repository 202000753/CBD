...
CREATE PROCEDURE insertNewStudent (@mail nvarchar(70),
									@name nvarchar (60),
									@gender nvarchar,
									@birthdate datetime,
									@addressType bit,
...
CREATE PROCEDURE insertNewSubject (@name nvarchar (60))
as
insert into ActiveData.Subject (Name) values (@name)
go

CREATE PROCEDURE insertRowCloosed_Course (@SchoolYear smallint,	@SubjectID int,	@StudentID int, @Failures tinyint, @Absences tinyint, @P1_grade tinyint, @P2_grade tinyint, @P3_grade tinyint)
as
...
CREATE PROCEDURE registerStudentInSubject (@studentID int, @subjectID int, @failures tinyint) --REGISTA UM ALUNO EM UMA DISCIPLINA NO ANO ACTUAL
as
BEGIN
...
CREATE PROCEDURE startSchoolYear (@year smallint)
as
BEGIN
	SELECT *
...
CREATE PROCEDURE endSchoolYear (@year smallint) --FECHA O ANO ESCOLAR, RECEBE O ANO
as
BEGIN
...
CREATE PROCEDURE migrateOldData -- CORRER DPS DA QUERY Q POPULA O SCHEMA OLD DATA COM A OLD DATA DATABASE
as
BEGIN
